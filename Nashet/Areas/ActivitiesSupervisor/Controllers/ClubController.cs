...
namespace Nashet.Areas.ActivitiesSupervisor.Controllers
{
    ...
    public class ClubController : Controller
    {
        ...
        {
            var clubs = SiteGuid.HasValue
                ? await _ClubDomain.GetClubBySiteGuid(SiteGuid.Value)
                : await _ClubDomain.GetClub();

            var sites = await _SiteDomain.GetSite();

            // تحويل الأندية لتضمين اسم الجهة مباشرة
            var clubViewModels = clubs.Select(c => new
            {
                Club = c,
                SiteName = sites?.FirstOrDefault(s => s.SiteId == c.SiteId)?.SiteNameAR ?? "غير محدد"
            }).ToList();

            ViewBag.Sites = sites;
            ViewBag.ClubViewModels = clubViewModels;
